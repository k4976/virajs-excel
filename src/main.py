...
def load_symbol_details():
    ...
    indTok = symbol_df[symbol_df.tradingsymbol == INDEX_MAP.get(INDEX)].instrument_token.tolist()
    if len(indTok) == 0:
        indTok = None
        print(f"Can't find InstrumentToken to subscribe {INDEX}.")

    # Filtering symbol by Index & 1st Expiry only. 
    d = OptionChain(df_symbol=symbol_df, index=INDEX, indexToken=indTok, depth=depth)
    optdf = d.symbols
    ...
    if indTok:
        indTok = indTok[0]
        # Subscribing Index.
        WS.subscribe(tokens=[indTok])
        # Getting, LTP of the index.
        ltp = WS.ltpStore.get_ltp(indTok)
        stt = time.time()
    ...
    symSheet = EXCEL_BOOK.sheets("SYMBOL_DETAILS")
    symSheet.range("A1:O200").value = None
    # Saving Data To Symbol Sheet.
    symSheet.range("A1").options(index=False, header=True).value = optdf
    symbol_df = optdf
    ...
    try:
        ...
        if data:
            for i, candle in enumerate(data):
                ...
                dataRet[0][i] = candle.get('date', '-')
                dataRet[1][i] = candle.get('low', '-')
        return dataRet
    except Exception as e:
        show_msg(e)
        print_exc()
...
def optionChainLtp():
    ...
    symSheet = EXCEL_BOOK.sheets('SYMBOL_DETAILS')
    if symSheet:
        ltpColumn = symSheet.range('E2').expand('down')
    else:
        print(f'Cant load excel sheet: SYMBOL, {symSheet}')
    

    # Getting all tokens from symbol list & subscribing them.
    tokenStrikes = symbol_df.instrument_token.tolist()
    WS.subscribe(tokens=tokenStrikes)

    # Now, updating Last_price column regularly.
    while not shutdown:
        dltp = []
        for i in symbol_df.itertuples():
            ltp = WS.ltpStore.get_ltp(i.instrument_token)
            if ltp: ltp = ltp.ltp
            else: ltp = '-'
            dltp.append([ltp])
        # Updating data in Excel.
        ltpColumn.value = dltp
        # Time to sleep
        ttm = 1
        time.sleep(ttm)
...
def get_live():
    ...
    while not shutdown:
        try:            
            ...
            # Tick Processing
            tick = WS.ltpStore.get_ltp(instrument_token)
...
def StartThread():
    try:
        ...
        global threads
        threads = []
        target = (get_live, get_orders, get_positions, get_holdings, get_funds, optionChainLtp)
        for t in target:
...
def main():
    ...
    try:
        ...
        if api:
            ...
            check_excel(), load_symbol_details()
